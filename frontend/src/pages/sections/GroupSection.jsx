  ...
  return (
    <div>
      <div className="page-header">
        <h1 className="page-title">グループ</h1>
      </div>
        ...
        <>
          <div className="card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              ...
              <span className="tag tag-green">{group.role === 'owner' ? 'オーナー' : 'メンバー'}</span>
            </div>

            <div style={{ marginTop: 12 }}>
              <div className="card-title">メンバー ({group.members.length}/7)</div>
              {group.members.map((m) => (
          ...
          {/* Invite (owner only) */}
          {group.role === 'owner' && group.type === 'family' && group.total_slots_used < 7 && (
            <div className="card">
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                >
                  <UserPlus size={15} strokeWidth={2} style={{ marginRight: 4 }} />
                  招待
                </button>
              </div>
              {inviteUrl && (
                <div style={{ marginTop: 8, padding: 8, background: 'var(--bg)', borderRadius: 8 }}>
                  <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4 }}>招待リンク（72時間有効・1回限り）:</div>
                  <div style={{ fontSize: 12, wordBreak: 'break-all', color: 'var(--green-800)' }}>{inviteUrl}</div>
                  <button
                    className="btn btn-outline"
                    style={{ fontSize: 12, marginTop: 8, padding: '4px 12px' }}
                    onClick={() => navigator.clipboard.writeText(inviteUrl)}
                  >
                    <Copy size={13} strokeWidth={2} style={{ marginRight: 4 }} />
                    コピー
                  </button>
                </div>
              )}
              ...
              {!showCreate ? (
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <button className="btn btn-outline" onClick={() => setShowCreate(false)}>キャンセル</button>
                  </div>
                </div>
              )}
            </div>
          ...
          {group.type === 'family' && (
            <div style={{ marginTop: 16 }}>
              <button
                ...
                onClick={() => {
                  if (window.confirm('グループから脱退しますか？')) leaveMutation.mutate();
                }}
